...
- ראשי (המסך הראשי): מצב כל המכשירים והערוצים (נקודה ירוקה = מחובר), הדלקה/כיבוי מיידי בלחיצה.
...
export async function answerSupportQuestion({ userId, text }) {
  ...
  const response = await client.messages.create({
    ...
    max_tokens: 1024,
    // Thinking off: with it on (Sonnet 5 default) thinking tokens can eat the
    // 1024 budget before the JSON answer.
